...
import InkopWeekTwo from "../components/InkopWeekTwo"
...
const Week2 = () => {
  ...
  `)
  const sortedPosts2 = data.menyV2.edges.sort(sortingFunction)
  const { dayOfWeek, weekDays, weekNumber } = getCurrentWeekInfo()
  const [isModal, setIsModal] = useState(false)

  return (
    <Grid container spacing={2}>
      {sortedPosts2.reverse().map(edge => {
        ...
        return (
          <Grid key={edge.node.rtt} item xs={6} sm={6} md={4} lg={4} xl={4}>
            <Link className="menu-link" to={`/menyv2/${edge.node.slug}/`}>
              <Box
                key={edge.node.dag}
                alignItems="center"
                border={
